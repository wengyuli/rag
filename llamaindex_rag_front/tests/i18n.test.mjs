...
test('restores the selected language and updates accessible document metadata', () => {
  ...
  assert.equal(document.title, 'Enterprise Digital Asset AI Management Platform')
})

test('switches reactively and persists only valid supported languages', async () => {
  setLocale('zh')
  await nextTick()
  assert.equal(t('企业数字资产 AI 管理平台'), '企业数字资产 AI 管理平台')
  assert.equal(intlLocale.value, 'zh-CN')
  assert.equal(document.documentElement.lang, 'zh-CN')
  assert.equal(document.title, '企业数字资产 AI 管理平台')
