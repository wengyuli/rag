...
语言切换只影响界面；文件名、上传资料、用户创建的部门名称、已有问题和回答保持原文。AI 查询可直接使用中文或英文提问。
...
已验证：架构页在菜单内和独立路径均可打开，中英文产品/技术图切换通过；已实际下载中文产品图和英文技术图，SVG 为独立矢量文件，无需外部图片或脚本。6 项语言检查与生产构建通过。

## 文件列表与资源监控

媒体库使用列表展示，包含媒体类型图标、文件名、大小、上传时间、所属媒体库/上传者、解析状态与操作。保留名称搜索、类型筛选、预览、失败重试和删除；详细摘要与解析提示可按行展开。
...
管理员和普通成员均可从左侧“系统架构”查看 **产品流程** 与 **技术架构** 两张图，了解资料上传、解析、检索问答，以及各个本地服务之间的关系。页面支持下载 SVG，图表和说明随中英文切换同步更新。

也可独立打开 [本机系统架构页](http://127.0.0.1:8090/architecture)。该入口无需登录，仅展示静态介绍，不读取业务资料或调用业务 API；它沿用本机 8090 端口与回环绑定配置，不会自动发布到公网。
...
- 资料搜索目前按文件名筛选；语义内容检索在 AI 查询中进行。暂不包括在线文件编辑、音频单独上传、全文审阅审批或视频剪辑。
